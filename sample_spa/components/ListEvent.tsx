...
const ListEvent = ({}) => {
    const {Events, updateEvents, deleteEvent, removeEvent} = useContext(EventContext);
    ...
    return (
        <div className="text-center">
            ...
            <ul className="max-w-md space-y-1 text-center mb-4">
                {Events.map((Event:Event, index:number) => ( // Mapeamento dos eventos da lista de eventos do navegador para exibição na página.
                    <>{<li className='font-light mt-3'> Nome: {Event.eventName} | Data: {Event.data} | Time: {Event.time} | Descrição: {Event.description}
                        <button onClick={() => {
